...
import { createProgressReport } from "./graphql/mutations";
const client = generateClient();
export default function ProgressReportCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setReportDetails(initialValues.reportDetails);
    setErrors({});
  };
  const validations = {
    uuid: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: createProgressReport.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
      ...
      {...getOverrideProps(overrides, "ProgressReportCreateForm")}
      {...rest}
    >
      <TextField
        label="Uuid"
